 ...
 */
class TimerManager{
	private _handlers:Array<TimerHandler> = null;
    private _delHandlers:Array<TimerHandler> = null;
	private _currTime:number = null;
	private _currFrame:number = null;
	private _count:number = null;
    private _timeScale:number = null;

    private static _inst:TimerManager;

    public static i():TimerManager{
        if(this._inst == null)
        {
...
//                App.DebugUtils.start(handler.method.toString());
                handler.method.call(handler.methodObj, (this._currTime - handler.dealTime)*this._timeScale,handler.alreadyRepeatCount);
//                App.DebugUtils.stop();
...
class TimerHandler {
	...
	/**已经执行的次数 */
	public alreadyRepeatCount:number = 1;
	/**是否用帧率*/
	public userFrame:boolean = null;
	/**执行时间*/
	public exeTime:number = 0;
	/**处理函数*/
	public method:Function = null;
    /**处理函数所属对象*/
    public methodObj:any = null;
	/**完成处理函数*/
	public complateMethod:Function = null;
    /**完成处理函数所属对象*/
    public complateMethodObj:any = null;
    /**上次的执行时间*/
